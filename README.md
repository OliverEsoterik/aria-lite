...
* `make run`: The standard command to run your daily scripts. It ensures the database is running in the background and executes the ETL pipeline. Optionally limit tickers: `make run MAX=100`. By default processes **all** tickers (US + European). Add `US=1` to process only US tickers: `make run US=1 MAX=2500`.
