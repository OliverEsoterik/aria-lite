...
* `make populate-market-cap`: Fetches market cap from yfinance for all tickers in `dim_entities` and stores it in the database. **Required once before the market-cap floor works.** Default skips tickers that already have market_cap. Has retry with backoff for yfinance rate limiting. Does not need daily re-runs — the $500M floor is conservative enough that daily price moves don't materially change the pool.
* `make populate-market-cap-refresh`: Force re-fetch market cap for ALL tickers.
